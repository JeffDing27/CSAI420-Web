...
export class ConsentedClinicianService {
  ...
  async getConsentedClinicians(customer: string): Promise<{clinicianUsername: string}[]> {
    const clinicians = await this.repo.findByCustomer(customer);
    return clinicians.map(c => ({ clinicianUsername: c.clinicianUsername }));
  }

  async addConsentedClinician(customer: string, clinicianUsername: string): Promise<void> {
