...
export async function POST(request: Request) {
  ...
  if (!email || !password || !birthDate || !phone || !firstName || !lastName) {
    return new Response("Missing required fields for registration", { status: 400 });
  }
  ...
  if (response.status === 200 || response.status === 201) {
    ...
    return NextResponse.json({ message: "Registration successful (Mock fallback)" }, { status: 201 });
  }
}
