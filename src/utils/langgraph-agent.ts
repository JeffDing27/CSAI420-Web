...
function callModel(state: typeof StateAnnotation.State) {
  if (process.env.OPENAI_ENABLED !== 'true' || !process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY.startsWith('sk-dummy')) {
    // Mock Response
  ...
  return { messages: [new AIMessage("OpenAI real implementation placeholder")] };
}

function retrieveContext(state: typeof StateAnnotation.State) {
  const lastMessage = state.messages[state.messages.length - 1];
